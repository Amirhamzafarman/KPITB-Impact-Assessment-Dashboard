...
# 2. MVRS Motor Vehicle System (Excise Dept)
...
# 3. Driving License System (Transport Dept)
driving_total_apps = 4233257
driving_services = [
    ("Permanent Driving License", 2496825, 149809500),
    ("Learner Permit", 554189, 27709450),
    ("License Renewal & Endorsement", 1171719, 40000000),
    ("International Driving Permit (IDP)", 10524, 6518260)
]

# 4. Wildlife & Hunting Licensing (Forestry & Wildlife Dept)
hunting_total_apps = 37488
hunting_services = [
    ("New Hunting License", 37068, 8500000),
    ("New Possession License", 409, 300087),
    ("Hunting Permit & Game Reserve Permits", 11, 40000)
]
...
def generate_system_records(system_name, service_list, target_apps, female_ratio):
        ...
        for yr in years:
            ...
            for m in months:
                for s_name, s_cnt, s_rev_total in service_list:
                    ...
                    female_cnt = 1 if (rec_apps > 40 and random.random() < female_ratio) else 0
...
# 1. Generate Arms records
generate_system_records("Arms & Licensing", arms_services, arms_total_apps, 0.003)

# 2. Generate MVRS records
generate_system_records("MVRS", mvrs_services, mvrs_total_apps, 0.035)

# 3. Generate Driving License records
generate_system_records("Driving Licenses", driving_services, driving_total_apps, 0.005)

# 4. Generate Wildlife & Hunting records
generate_system_records("Wildlife & Hunting", hunting_services, hunting_total_apps, 0.001)

print(f"Generated {len(cube_records)} records in kpk_4system_cube.json")

with open("/Users/hamza/Documents/KPITB/RTS/DIGITAL IMPACT/kpk_4system_cube.json", "w", encoding="utf-8") as f:
    json.dump(cube_records, f)

print("✔ Saved kpk_4system_cube.json successfully!")
